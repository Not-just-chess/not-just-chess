...
RSpec.describe Pawn, type: :model do
  describe 'valid_move?' do
    ...
    it 'verifies invalid moves' do
      game = FactoryBot.create(:game)
      pawn_4_4 = FactoryBot.create(:pawn, x_position: 4, y_position: 4, game_id: game.id, color: true)
      pawn_2_3 = FactoryBot.create(:pawn, x_position: 2, y_position: 3, game_id: game.id, color: true)
      pawn_4_8 = FactoryBot.create(:pawn, x_position: 4, y_position: 8, game_id: game.id, color: true)
      FactoryBot.create(:bishop, x_position: 2, y_position: 4, game_id: game.id, color: false)

      # Invalid Horizontal Move
      expect(pawn_4_4.valid_move?([5, 4])).to eq(false)

      # checking for a move that is obstructed
      expect(pawn_2_3.valid_move?([2, 5])).to eq(false)

      # checking for a move of the board
      expect(pawn_4_8.valid_move?([9, 9])).to eq(false)

      # checking for a non-diagonal/vertical/horizontal move
      expect(pawn_4_8.valid_move?([5, 6])).to eq(false)
    end
  end
end
